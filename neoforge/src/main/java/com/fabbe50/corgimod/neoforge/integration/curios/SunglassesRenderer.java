...
public class SunglassesRenderer implements ICurioRenderer {
    private final ResourceLocation SUNGLASSES_RESOURCE = TheCorgiMod.location("textures/models/curios/sunglasses_layer_1full.png");
    ...
    public <T extends LivingEntity, M extends EntityModel<T>> void render(ItemStack itemStack, SlotContext slotContext, PoseStack poseStack, RenderLayerParent<T, M> renderLayerParent, MultiBufferSource multiBufferSource, int i, float v, float v1, float v2, float v3, float v4, float v5) {
        ...
        if (entityModel instanceof HumanoidModel<T> humanoidModel) {
            ...
            humanoidModel.renderToBuffer(poseStack, vertex, i, OverlayTexture.NO_OVERLAY, FastColor.ARGB32.color(255, 255, 255, 255));
            if (itemStack.hasFoil()) {
                humanoidModel.renderToBuffer(poseStack, multiBufferSource.getBuffer(RenderType.armorEntityGlint()), i, OverlayTexture.NO_OVERLAY, FastColor.ARGB32.color(255, 255, 255, 255));
            }
            poseStack.popPose();
        }
    }
